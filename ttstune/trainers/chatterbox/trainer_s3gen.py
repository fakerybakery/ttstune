...
from chatterbox.models.s3tokenizer import (
    S3_SR as S3_TOKENIZER_SR,
)  # S3Tokenizer operates at 16kHz
...
class S3GenModelArguments:
    model_name_or_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to base Chatterbox model"},
    )
    local_model_dir: Optional[str] = field(
        default=None,
        metadata={"help": "Path to local Chatterbox model directory"},
    )
    cache_dir: Optional[str] = field(
        default=None,
        metadata={"help": "HF cache directory"},
    )
    # S3Gen specific finetuning args
    freeze_speaker_encoder: bool = field(
        default=True,
        metadata={"help": "Freeze S3Gen's internal speaker encoder (CAMPPlus)."},
    )
    freeze_s3_tokenizer: bool = field(
        default=True,
        metadata={"help": "Freeze S3Gen's internal S3Tokenizer."},
    )
...
class S3GenDataArguments:
    dataset_name: Optional[str] = field(
        default=None,
        metadata={"help": "HF Dataset name"},
    )
    dataset_config_name: Optional[str] = field(
        default=None,
        metadata={"help": "HF Dataset config name"},
    )
    ...
    eval_split_name: Optional[str] = field(
        default="validation",
        metadata={"help": "Eval split name"},
    )
    audio_column_name: str = field(
        default="audio",
        metadata={"help": "Audio column in dataset"},
    )
    # No text column needed for S3Gen training directly, but might be in dataset

    max_speech_token_len: int = field(
        default=750,
        metadata={"help": "Max S3 speech tokens for target sequence."},
    )
    max_mel_len: int = field(
        default=1500,
        metadata={"help": "Max mel frames for target mel (max_speech_token_len * 2)"},
    )
    # For CausalMaskedDiffWithXvec, we need prompt_tokens and prompt_feats for conditioning
    prompt_audio_duration_s: float = field(
        default=3.0,
        metadata={"help": "Duration of audio for prompt_token and prompt_feat."},
    )

    eval_split_size: float = field(
        default=0.01,
        metadata={"help": "Eval split fraction"},
    )  # Increased default
    ignore_verifications: bool = field(
        default=False,
        metadata={"help": "Ignore dataset verifications."},
    )
...
class S3GenFineTuningDataset(Dataset):
    def __init__(
        ...
        self.s3gen_model = (
            s3gen_model  # Contains tokenizer, mel_extractor, speaker_encoder
        )
        self.dataset_source = hf_dataset

        self.s3_tokenizer_sr = S3_TOKENIZER_SR  # 16kHz for S3Tokenizer
        self.s3_gen_native_sr = (
            S3GEN_SR  # 24kHz for mel spectrograms and CAMPPlus input
        )

        self.prompt_audio_samples_16k = int(
            data_args.prompt_audio_duration_s * self.s3_tokenizer_sr,
        )
        self.prompt_audio_samples_24k = int(
            data_args.prompt_audio_duration_s * self.s3_gen_native_sr,
        )
    ...
    def _load_and_preprocess_audio(self, audio_data_from_hf):
        ...
        if isinstance(audio_data_from_hf, str):
            try:
                waveform, original_sr = torchaudio.load(audio_data_from_hf)
            except Exception:
                return None, None
        elif (
            isinstance(audio_data_from_hf, dict)
            and "array" in audio_data_from_hf
            and "sampling_rate" in audio_data_from_hf
        ):
        ...
        if waveform is None or original_sr is None or waveform.numel() == 0:
            return None, None
        ...
        if original_sr != self.s3_tokenizer_sr:
            ...
            wav_16k_tensor = (
                waveform.clone()
            )  # Clone to avoid modifying dataset source waveform
        ...
        return wav_16k_tensor.squeeze(0), wav_24k_tensor.squeeze(
            0,
        )  # Return (L,) tensors
    ...
    def __getitem__(self, idx) -> Optional[dict[str, torch.Tensor]]:
        ...
        if (
            wav_16k_tensor is None
            or wav_24k_tensor is None
            or wav_16k_tensor.numel() == 0
            or wav_24k_tensor.numel() == 0
        ):
            return None
        ...
        try:
            target_mel = (
                self.s3gen_model.mel_extractor(wav_24k_tensor.unsqueeze(0))
                .squeeze(0)
                .transpose(0, 1)
            )  # (T_mel, F)
        ...
        try:
            speech_tokens_batch, speech_token_lengths_batch = (
                self.s3gen_model.tokenizer.forward(
                    [wav_16k_tensor.numpy()],
                    max_len=self.data_args.max_speech_token_len,
                )
            )
            if speech_tokens_batch is None or speech_token_lengths_batch is None:
                return None
            target_s3_tokens = speech_tokens_batch.squeeze(0)  # (T_tokens_s3)
            speech_token_len = torch.tensor(
                target_s3_tokens.size(0),
                dtype=torch.long,
                device="cpu",
            )
            ...
            if target_mel.size(0) != 2 * target_s3_tokens.size(0) and target_mel.size(
                0,
            ) // 2 < target_s3_tokens.size(0):
                target_s3_tokens = target_s3_tokens[: target_mel.size(0) // 2]
                speech_token_len = torch.tensor(
                    target_s3_tokens.size(0),
                    dtype=torch.long,
                )
            elif target_mel.size(0) // 2 > target_s3_tokens.size(
                0,
            ):  # Pad tokens if mel is longer due to truncation
                pad_size = target_mel.size(0) // 2 - target_s3_tokens.size(0)
                target_s3_tokens = F.pad(
                    target_s3_tokens,
                    (0, pad_size),
                    value=0,
                )  # Pad with 0
            ...
            logger.error(
                f"Item {idx}: Error tokenizing target speech: {e}",
                exc_info=True,
            )
            return None
        ...
        try:
            ...
            logger.error(
                f"Item {idx}: Error getting speaker_embedding: {e}",
                exc_info=True,
            )
        ...
        if (
            prompt_wav_16k_segment.numel() == 0 or prompt_wav_24k_segment.numel() == 0
            ...
            max_flow_prompt_token_len = self.s3gen_model.flow.encoder.hp.get(
                "prompt_token_max_len",
                75,
            )  # Example, should come from flow model config
            ...
            try:
                ...
                max_flow_prompt_token_len = getattr(
                    self.s3gen_model.flow.encoder,
                    "prompt_token_max_len",
                    75,
                )  # Check actual attribute if exists
                prompt_s3_tokens_batch, prompt_s3_token_lengths_batch = (
                    self.s3gen_model.tokenizer.forward(
                        [prompt_wav_16k_segment.numpy()],
                        max_len=max_flow_prompt_token_len,
                    )
                )
                if prompt_s3_tokens_batch is None:
                    return None
                ...
                prompt_mel = (
                    self.s3gen_model.mel_extractor(prompt_wav_24k_segment.unsqueeze(0))
                    .squeeze(0)
                    .transpose(0, 1)
                )  # (T_mel_prompt, F)
                ...
                if prompt_mel.size(0) // 2 < prompt_s3_tokens.size(0):
                    ...
                    prompt_s3_token_len = torch.tensor(
                        prompt_s3_tokens.size(0),
                        dtype=torch.long,
                    )

            except Exception as e:
                logger.error(
                    f"Item {idx}: Error processing prompt features: {e}",
                    exc_info=True,
                )
                return None
...
class S3GenDataCollator:
    ...
    mel_pad_value: float = (
        0.0  # Or whatever the flow model expects for padded mel frames
    )
    ...
    def __call__(
        ...
        padded_speech_tokens = torch.stack(
            [
                F.pad(s, (0, max_len_st - s.size(0)), value=self.speech_token_pad_id)
                for s in speech_tokens
            ],
        )
        ...
        speech_feats[0].size(1)  # F dimension
        padded_speech_feats = torch.stack(
            [
                F.pad(s, (0, 0, 0, max_len_sf - s.size(0)), value=self.mel_pad_value)
                for s in speech_feats
            ],  # Pads last dim first (T_mel)
        ...
        target_prompt_token_len = (
            max(pt.size(0) for pt in prompt_tokens) if prompt_tokens else 0
        )
        ...
        padded_prompt_tokens = torch.stack(
            [
                F.pad(
                    pt,
                    (0, target_prompt_token_len - pt.size(0)),
                    value=self.speech_token_pad_id,
                )
                for pt in prompt_tokens
            ],
        )
        prompt_token_lens = torch.stack(
            [f["prompt_token_len_input"] for f in valid_features],
        )
        ...
        target_prompt_mel_len = (
            max(pf.size(0) for pf in prompt_feats) if prompt_feats else 0
        )
        ...
        padded_prompt_feats = torch.stack(
            [
                F.pad(
                    pf,
                    (0, 0, 0, target_prompt_mel_len - pf.size(0)),
                    value=self.mel_pad_value,
                )
                for pf in prompt_feats
            ],
        )
...
class S3GenFlowForFineTuning(torch.nn.Module):
    def __init__(
        self, s3gen_token_to_mel: S3Token2Mel
    ) -> None:  # Pass S3Token2Mel instance
        super().__init__()
    ...
    def forward(
        ...
        projected_speaker_emb = self.flow_model.spk_embed_affine_layer(
        ...
        h_encoded_full, _ = self.flow_model.encoder(
            input_token_embed,
            full_input_token_lens,
        )
        ...
        target_mels_for_cfm_x = speech_feat.transpose(
            1,
            2,
        ).contiguous()  # (B, F, T_target_mel_collator)
        target_mel_mask = (
            ~self.make_pad_mask(speech_feat_len, max_len=target_mels_for_cfm_x.size(2))
        ).unsqueeze(1)
        ...
        if T_prompt_mel_collator > T_target_mel_collator:
            ...
            padded_prompt_mels_for_cond = original_prompt_mels[
                :,
                :,
                :T_target_mel_collator,
            ]
            logger.warning(
                f"Prompt mel length ({T_prompt_mel_collator}) was longer than target mel length ({T_target_mel_collator}). Prompt was truncated.",
            )
        else:
            ...
            padded_prompt_mels_for_cond = F.pad(
                original_prompt_mels,
                (0, padding_size),
                mode="constant",
                value=0,
            )
        ...
        num_prompt_mel_frames_collator = (
            T_prompt_mel_collator  # Using the original prompt length for slicing mu
        )
        ...
        if slice_start_idx >= mu_full_for_cfm.size(2):
            logger.error(
                f"MU SLICING ERROR: mu_full_for_cfm (len {mu_full_for_cfm.size(2)}) too short for prompt part (len {slice_start_idx})",
            )
            mu_conditioning_for_target_mels = torch.zeros_like(
                target_mels_for_cfm_x.float().expand(-1, mu_full_for_cfm.size(1), -1),
            )
        else:
            ...
            if current_raw_target_slice_len < num_target_mel_frames_collator:
                padding_needed = (
                    num_target_mel_frames_collator - current_raw_target_slice_len
                )
                mu_conditioning_for_target_mels = F.pad(
                    mu_target_raw_slice,
                    (0, padding_needed),
                )
                ...
                mu_conditioning_for_target_mels = mu_target_raw_slice[
                    :,
                    :,
                    :num_target_mel_frames_collator,
                ]
            else:
                mu_conditioning_for_target_mels = mu_target_raw_slice
...
def main() -> None:
    ...
    parser = HfArgumentParser(
        (S3GenModelArguments, S3GenDataArguments, CustomTrainingArguments),
    )
    ...
    if model_args.local_model_dir:
        chatterbox_model = ChatterboxTTS.from_local(
            ckpt_dir=model_args.local_model_dir,
            device="cpu",
        )
    else:
        repo_to_download = (
            model_args.model_name_or_path or REPO_ID
        )  # Fallback to default REPO_ID
    ...
    s3gen_token_to_mel_part: S3Token2Mel = (
        s3gen_model_to_finetune  # S3Gen inherits from S3Token2Wav which inherits from S3Token2Mel
    )
    ...
    if data_args.dataset_name:
        ...
        eval_hf_dataset = (
            raw_datasets.get(data_args.eval_split_name)
            if data_args.eval_split_name
            else None
        )
        if (
            training_args.do_eval
            and not eval_hf_dataset
            and data_args.eval_split_size > 0
            and len(train_hf_dataset) > 1
        ):
            split_dataset = train_hf_dataset.train_test_split(
                test_size=data_args.eval_split_size,
        ...
        msg = "S3Gen finetuning currently requires a Hugging Face dataset_name."
        raise ValueError(
            msg,
        )  # Simpler for now

    train_dataset = S3GenFineTuningDataset(
        data_args,
        s3gen_model_to_finetune,
        train_hf_dataset,
    )
    eval_dataset = None
    if eval_hf_dataset and training_args.do_eval:
        eval_dataset = S3GenFineTuningDataset(
            data_args,
            s3gen_model_to_finetune,
            eval_hf_dataset,
        )
    ...
    def compute_metrics_s3gen(eval_preds):
        metrics = {}
        if (
            isinstance(eval_preds.predictions, tuple)
            and len(eval_preds.predictions) >= 1
        ):
            ...
            if (
                len(eval_preds.predictions) > 1
                and eval_preds.predictions[1] is not None
            ):
                metrics["eval_cfm_loss"] = float(np.mean(eval_preds.predictions[1]))
            if (
                len(eval_preds.predictions) > 2
                and eval_preds.predictions[2] is not None
            ):
    ...
    trainer_instance = Trainer(
        ...
        compute_metrics=(
            compute_metrics_s3gen if training_args.do_eval and eval_dataset else None
        ),
        callbacks=callbacks if callbacks else None,
    )
    if training_args.label_names is None:
        trainer_instance.label_names = []  # Model handles its own targets
    ...
    if training_args.do_train:
        ...
        logger.info(
            "Saving finetuned S3Gen (flow part) model weights for ChatterboxTTS...",
        )
        ...
        output_s3gen_safetensor_path = (
            Path(training_args.output_dir) / "s3gen.safetensors"
        )
        from safetensors.torch import save_file

        save_file(finetuned_s3gen_state_dict, output_s3gen_safetensor_path)
        logger.info(
            f"Finetuned S3Gen model weights saved to {output_s3gen_safetensor_path}",
        )
